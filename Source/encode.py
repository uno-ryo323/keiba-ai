...
class encode():
    def make_encode_pickle():
        keiba_data = pd.read_csv("C:\\keibaAI\\Data\\netKeiba\\result\\race_jra2.1.csv", sep=",", encoding="cp932")
        columns = ["class","course","turn","weather","state","sex","kanri","jockey",\
                    "trainer","banusi","breeder","father","father_father","father_mother","mother",\
        ...
        for column in columns:
            ...
            pickle.dump(le, open("C:\\KeibaAI\\Source\\encode\\Ver1.1\\" + column +  ".pickle", 'wb'))
    ...
    def join_netkeiba_target():
        ...
        for i in range(0,length):
            ...
            netkeiba_data.loc[i,'corner_count'] = target_data.loc[index,'corner_count']
            netkeiba_data.loc[i,'course_class'] = target_data.loc[index,'course_class']
            ...
            netkeiba_data.loc[i,'race_type_code'] = target_data.loc[index,'race_type_code']
            netkeiba_data.loc[i,'race_symbol_code'] = target_data.loc[index,'race_symbol_code']
            netkeiba_data.loc[i,'weight_type_code'] = target_data.loc[index,'weight_type_code']
            netkeiba_data.loc[i,'race_type_code'] = target_data.loc[index,'race_type_code']
            netkeiba_data.loc[i,'weight_type_code'] = target_data.loc[index,'weight_type_code']
            netkeiba_data.loc[i,'blinkers'] = target_data.loc[index,'blinkers']
            netkeiba_data.loc[i,'rank_offical'] = target_data.loc[index,'rank_offical']
            netkeiba_data.loc[i,'rank_arrival'] = target_data.loc[index,'rank_arrival']
            netkeiba_data.loc[i,'abnormal_code'] = target_data.loc[index,'abnormal_code']
            ...
            netkeiba_data.loc[i,'mother_father_type'] = target_data.loc[index,'mother_father_type']
            #if i % 10000 == 0:    
