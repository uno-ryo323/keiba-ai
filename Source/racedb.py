...
class raceDB():    
    ...
    PATH = "C:\\keibaAI\\Data\\netKeiba\\result\\race_all.csv"
    #PATH = "C:\\keibaAI\\Data\\netKeiba\\result\\race_jra2.csv"
    ...
    def get_horse_info(horse_link,race_id):
        ...
        for horse_number in horse_link:
            ...
            time.sleep(0.5)
            res = raceDB.session.get("https://db.netkeiba.com" + horse_number)
            ...
            for i in range(1,5):
                if "調教師" in str(horse_prof[i]) or "馬主" in str(horse_prof[i]) or "生産者" in str(horse_prof[i]):
    ...
    def get_jockey_info(jockey_link):
        ...
        for jockey_number in jockey_link:
            ...
            time.sleep(0.5)
            #print(jockey_number)
            res = raceDB.session.get("https://db.netkeiba.com" + jockey_number)
            soup = BeautifulSoup(res.content, 'lxml')
    ...
    def horse_list_split(HorseList):
        ...
        HorseList = HorseList.replace("着順枠番馬番馬名性齢斤量騎手タイム着差ﾀｲﾑ指数通過上り単勝人気馬体重調教ﾀｲﾑ厩舎ｺﾒﾝﾄ備考調教師馬主賞金(万円)","")
        ...
        while i+3 < len(tmp):    
            ...
            zougen = tmp[i+14].replace(str(weight), "").replace("(", "").replace(")","")
            remarks = tmp[i+17]
            kanri = ""
            if tmp[i+18] != "":
                kanri = tmp[i+18][0]
            ...
            Race_lists.append(str(rank_race) + "," + str(number_frame) + "," + str(number_horse) + "," + horse_name \
                    + "," + horse_sex + "," + str(horse_old) + "," + str(handiycap) + "," + str(weight) \
                    + "," + str(zougen) +  "," + str(goal_time) \
                    + "," + str(rank_popular) + "," + str(odds) + "," + str(agari) + "," + str(corner_position1) + "," + str(corner_position2) + "," + str(corner_position3) + "," + str(corner_position4) \
                    + "," + str(time_index) + "," + kanri + "," + remarks)
            i = i + 21
            count = count +1
        return Race_lists
    ...
    def get_race_result(race_ids):
        ...
        for race_id in race_ids:
            ...
            race_info1 = [[str(s).replace('amp;', '') for s in text] for text in race_info1]
            race_info2 = [str(s).replace('amp;', '') for s in race_info2]
            horse_info = [[str(s).replace('amp;', '') for s in text] for text in horse_info]
            ...
            for o in range(0,horse_len):
                result_str = race_id + "," + str(race_info1[0][0]) + "," + str(race_info1[0][1]) + "," + str(race_info1[0][2])
                result_str = result_str + "," + race_name + "," + place
                result_str = result_str + "," + str(race_info1[1][0]) + "," + str(race_info1[1][1]) + "," + race_num
                result_str = result_str + "," + str(race_info1[2][0]) + "," + str(race_info1[3][0]) + ","  + str(race_info1[3][1])
                result_str = result_str + "," + str(race_info2[0]) + ","  + str(race_info2[1]) + ","  + str(race_info2[2]) + ","  + str(race_info2[3]) + ","  + str(race_info2[4])
                result_str = result_str + "," + str(horse_len) + "," + result_table[o] + "," + str(types[o])
                result_str = result_str + "," + jockey_info[o] + "," + horse_info[0][count1] + "," + horse_info[0][count1+1] + "," + horse_info[0][count1+2]
                result_str = result_str + "," + horse_info[1][count2] + "," + horse_info[1][count2+1] + "," + horse_info[1][count2+2] + "," + horse_info[1][count2+3] \
                    + "," + horse_info[1][count2+4] + "," + horse_info[1][count2+5]
                result_str = result_str + "," + horse_info[2][o]
                result_str = result_str + "," + str(horse_info[3][count3]) + "," + str(horse_info[3][count3+1]) +  "," + str(horse_info[3][count3+2]) +  "," + str(horse_info[3][count3+3]) +  "," + str(horse_info[3][count3+4]) +  "," + str(horse_info[3][count3+5])
                #print(result_str)
                count1 = count1+3
                count2 = count2+6
                count3 = count3+6
