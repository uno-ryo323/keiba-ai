...
class keibaAIBatch:
    ...
    def get_race_day(year, month):
        ...
        data = pd.read_csv("C:\\KeibaAI\\Source\\Batch\\schedule_" + str(datetime.date(int(year),int(month),1).strftime('%Y%m')) + ".csv" , sep="," , encoding="cp932")
        batchPath = "C:\\KeibaAI\\Source\\Batch\\getRacelist.bat"
        for i in range(0,len(data)):
            print(data.loc[i])
            taskName = "getRaceList" + "_" + str(data.loc[i,'kaisai_date'])
            date = str(data.loc[i,'get_list_date'])
            command = "schtasks /Create /SC ONCE /TN " + taskName + " /TR \"" + batchPath + " 1 \"" + str(data.loc[i,'kaisai_date']) + " /ST 18:00 /SD " + str(datetime.date(int(date[0:4]),int(date[4:6]),int(date[6:])).strftime('%Y/%m/%d'))
            #print(command)
            os.system(command)
        #print(data)
    ...
    def get_race_list(date):
        ...
        batchPath1 = "C:\\KeibaAI\\Source\\Batch\\forecast.bat"
        batchPath2 = "C:\\KeibaAI\\Source\\Batch\\getRaceResult.bat"
        #事前データ取得
        for i in range(0,len(data)):
            #レース予想
            taskName = "forecast" + "_" + str(data.loc[i,'race_id'])
            time = str(data.loc[i,'startTime']).split(":")
            dt_start = datetime.datetime(int(date[0:4]),int(date[4:6]),int(date[6:]), int(time[0]), int(time[1]))
            dt_diff = datetime.timedelta(minutes=20)
            exeDate = dt_start - dt_diff
            command = "schtasks /Create /SC ONCE /TN " + taskName + " /TR \"" + batchPath1 + " 4 " + str(data.loc[i,'race_id']) + "\" /ST " + str(exeDate.strftime("%H:%M"))  + " /SD " + str(datetime.date(int(date[0:4]),int(date[4:6]),int(date[6:])).strftime('%Y/%m/%d'))
            #print(command)
            os.system(command)
            #結果確認
            taskName = "getRaceResult" + "_" + str(data.loc[i,'race_id'])
            exeDate = dt_start + dt_diff
            command = "schtasks /Create /SC ONCE /TN " + taskName + " /TR \"" + batchPath2 + " 2 " + str(data.loc[i,'race_id']) + "\" /ST " + str(exeDate.strftime("%H:%M"))  + " /SD " + str(datetime.date(int(date[0:4]),int(date[4:6]),int(date[6:])).strftime('%Y/%m/%d'))
            #print(command)
            os.system(command)
    ...
    def get_race_result():
        ...
        race_list = pd.read_csv("C:\\keibaAI\\Data\\netKeiba\\racelist\\" + kaisai_date + ".csv", sep="," , encoding="cp932",dtype={'race_id':str})
        rd = racedb.raceDB
