...
class KeibaAI:
    ...
    def make_model():
        ...
        keiba_data = pd.read_csv(
            RESULT_DIR / "race_jra2.3.csv", sep=",", encoding="cp932"
        )
        ...
        filename = MODEL_DIR / "Ver1.1" / "Place.sav"
        pickle.dump(model, open(filename, "wb"))
        KeibaAI.show_feature_value(X.drop("year", axis=1).columns, model)

    def forecast_race():
        # modelのインポート(ToDo)
        model_dir_v1 = MODEL_DIR / "Ver1.1"
        model1 = pickle.load(open(model_dir_v1 / "Win.sav", "rb"))
        model2 = pickle.load(open(model_dir_v1 / "Quinella.sav", "rb"))
        model3 = pickle.load(open(model_dir_v1 / "Place.sav", "rb"))

        # 予測データの読み込み
        test_data = pd.read_csv(
            "C:\\KeibaAI\\Data\\netKeiba\\result\\race_jra2.3.csv",
            sep=",",
            encoding="cp932",
        )
        ...
        result_data.to_csv(RESULT_DIR / "2021_forecast.csv", sep=",", encoding="cp932")
