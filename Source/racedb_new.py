...
class raceDB:
    ...
    PATH = str(RESULT_DIR / "race_jra(HorseInfo).csv")
    ...
    def calc_time_index(table, course, distance, place):
        ...
        for row in table:
            if raceDB.is_float(row["time_index"]):
                ...
                if (
                    ...
                    and (course == row["course"])
                    and (place in row["place"])
                ):
        ...
        return [
            ...
            str(time_index_ave3),
            str(time_index_ave5),
            str(time_index_d_ave),
            str(time_index_c_ave),
            str(time_index_p_ave),
            str(time_index_dc_ave),
            str(time_index_dcp_ave),
            str(time_index_max),
            str(time_index_min),
            str(time_index_d_max),
            str(time_index_c_max),
            str(time_index_p_max),
            str(time_index_dc_max),
            str(time_index_dcp_max),
        ]
    ...
    def get_horse_info(keiba_data):
        ...
        table = []
        res = raceDB.session.get(f"{URL_DB}/horse/{keiba_data['horse_id']}")
        ...
        if pre_race_results is not None:
            ...
            for i in range(start, row):
                date = (
                    pre_race_results[i * 28].contents[0].contents[0].replace("\xa0", "")
                )
                if len(pre_race_results[i * 28 + 1].contents[0].contents) != 0:
                    place = (
                        pre_race_results[i * 28 + 1]
                        .contents[0]
                        .contents[0]
                        .replace("\xa0", "")
                    )
                ...
                gate_number = (
                    pre_race_results[i * 28 + 7].contents[0].replace("\xa0", "")
                )
                horse_number = (
                    pre_race_results[i * 28 + 8].contents[0].replace("\xa0", "")
                )
                ...
                if len(pre_race_results[i * 28 + 13].contents) != 0:
                    hande = (
                        pre_race_results[i * 28 + 13].contents[0].replace("\xa0", "")
                    )
                else:
                    hande = ""
                course = (
                    pre_race_results[i * 28 + 14].contents[0][0].replace("\xa0", "")
                )
                distance = (
                    pre_race_results[i * 28 + 14].contents[0][1:].replace("\xa0", "")
                )
                if len(pre_race_results[i * 28 + 15].contents) != 0:
                    state = (
                        pre_race_results[i * 28 + 15].contents[0].replace("\xa0", "")
                    )
                else:
                    state = ""
                state_index = (
                    pre_race_results[i * 28 + 16]
                    .contents[0]
                    .replace("\n", "")
                    .replace("\xa0", "")
                )
            ...
            if len(table) == 0:
                horse_info_str = (
                    str(keiba_data["race_id"]) + "," + str(keiba_data["horse_id"])
                )
            else:
                ...
                place_performance = raceDB.place_performance(
                    table, keiba_data["place2"]
                )
                weather_performance = raceDB.weather_performance(
                    table, keiba_data["weather"]
                )
                race_performance = raceDB.race_performance(
                    table, keiba_data["race_name"]
                )
                gate_performance = raceDB.gate_performance(table)
                calc_odds_popular = raceDB.calc_odds_popular(table)
                jockey_performance = raceDB.jockey_performance(
                    table, keiba_data["jockey"]
                )
                hande_performance = raceDB.hande_performance(
                    table, keiba_data["handiy"]
                )
                distance_performance = raceDB.distance_performance(
                    table, keiba_data["distance"], keiba_data["course"]
                )
                state_performance = raceDB.state_performance(table, keiba_data["state"])
                calc_time_diff = raceDB.calc_time_diff(table)
                calc_time_index = raceDB.calc_time_index(
                    table,
                    keiba_data["course"],
                    keiba_data["distance"],
                    keiba_data["place2"],
                ...
                weight_performance = raceDB.weight_performance(
                    table, keiba_data["weight"]
                )
                ...
                horse_info_str = (
                    ...
                    + ","
                    + weather_performance[1]
                    + ","
                    + weather_performance[2]
                    + ","
                    + weather_performance[3]
                    + ","
                    + weather_performance[4]
                )
                horse_info_str = (
                    horse_info_str
                    + ","
                ...
                horse_info_str = (
                    ...
                    + ","
                    + gate_performance[0][1]
                    + ","
                    + gate_performance[0][2]
                    + ","
                    + gate_performance[0][3]
                    + ","
                    + gate_performance[0][4]
                )
                horse_info_str = (
                    horse_info_str
                    + ","
                    + gate_performance[1][0]
                    + ","
                    + gate_performance[1][1]
                    + ","
                    + gate_performance[1][2]
                    + ","
                    + gate_performance[1][3]
                    + ","
                    + gate_performance[1][4]
                )
                horse_info_str = (
                    horse_info_str
                    + ","
                    + gate_performance[2][0]
                    + ","
                    + gate_performance[2][1]
                    + ","
                    + gate_performance[2][2]
                    + ","
                    + gate_performance[2][3]
                    + ","
                    + gate_performance[2][4]
                )
                horse_info_str = (
                    horse_info_str
                    + ","
                ...
                horse_info_str = (
                    horse_info_str
                    + ","
                    + jockey_performance[0]
                    + ","
                    + jockey_performance[1]
                    + ","
                    + jockey_performance[2]
                    + ","
                    + jockey_performance[3]
                    + ","
                    + jockey_performance[4]
                )
                horse_info_str = (
                    horse_info_str
                    + ","
                ...
                horse_info_str = (
                    horse_info_str
                    + ","
                    + distance_performance[0][0]
                    + ","
                    + distance_performance[0][1]
                    + ","
                    + distance_performance[0][2]
                    + ","
                    + distance_performance[0][3]
                    + ","
                    + distance_performance[0][4]
                )
                horse_info_str = (
                    horse_info_str
                    + ","
                    + distance_performance[1][0]
                    + ","
                    + distance_performance[1][1]
                    + ","
                    + distance_performance[1][2]
                    + ","
                    + distance_performance[1][3]
                    + ","
                    + distance_performance[1][4]
                )
                horse_info_str = (
                    horse_info_str
                    + ","
                    + distance_performance[2][0]
                    + ","
                    + distance_performance[2][1]
                    + ","
                    + distance_performance[2][2]
                    + ","
                    + distance_performance[2][3]
                    + ","
                    + distance_performance[2][4]
                )
                horse_info_str = (
                    horse_info_str
                    + ","
    ...
    def main():
        keiba_data = pd.read_csv(RESULT_DIR / "race_jra.csv", sep=",", encoding="cp932")
        ...
        for i in range(0, length):
            ...
            print(
                keiba_data.loc[i, "race_id"], keiba_data.loc[i, "horse_id"], i, length
            )
