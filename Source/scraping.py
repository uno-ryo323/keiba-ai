...
class Scraping():
    ...
    def get_jockey_info(jockeys, session):
        ...
        for jockey in jockeys:
            ...
            if jockey_number is None:
                ...
                time.sleep(1)
                res = session.get("https://db.netkeiba.com" + jockey_number)
    ...
    def trim_table(table):
        ...
        table = table.replace("着順枠番馬番馬名性齢斤量騎手タイム着差ﾀｲﾑ指数通過上り単勝人気馬体重調教ﾀｲﾑ厩舎ｺﾒﾝﾄ備考調教師馬主賞金(万円)","")
        tmp = table.split("_")
        return tmp
    ...
    def make_record(race_id, table_data, race_info, horse_info, jockey_info):
        ...
        head_str = str(race_id) + "," + str(race_info['year']) + "," + str(race_info['month']) + "," + str(race_info['day']) \
                    + "," + race_info['race_name'] + "," + str(race_info['place']) + "," +str(race_info['kai']) + "," + str(race_info['day2']) \
                    + "," + str(race_info['race_num']) +"," + str(race_info['class']) + "," + str(race_info['mare_only']) \
                    + "," + str(race_info['handiy']) + "," + str(race_info['course']) + "," + str(race_info['turn']) + "," + str(race_info['distance']) + "," + race_info['weather'] + "," + race_info['state'] +"," + str(length)
        ...
        for i in range(0,length):
            ...
            zougen = table_data[21*i+14].replace(str(weight), "").replace("(", "").replace(")","")
            ...
            record = head_str
            record = record + "," + str(rank) + "," + str(waku) + "," + str(umaban) + "," + str(horse_name) + "," + str(sex) + "," + str(old) + "," + str(handiycap) + "," + str(weight) + "," + str(zougen) \
                     + "," + str(goal_time) + "," + str(rank_popular) + "," + str(odds) + "," + str(agari) 
            record = record + "," + str(corner_position1) + "," + str(corner_position2) + "," + str(corner_position3) + "," + str(corner_position4) 
            record = record + "," + str(time_index) + "," + str(kanri) + "," + str(remarks) + "," + str(leg_type) + "," + jockey_info[i]
            record = record  + ","  + horse_info[i]['trainer'] + ","  + horse_info[i]['banusi'] + "," + horse_info[i]['breeder'] \
                    + "," + horse_info[i]['father'] + "," + horse_info[i]['father_father'] + "," + horse_info[i]['father_mother'] + "," + horse_info[i]['mother'] \
                    + "," + horse_info[i]['mother_father'] +  "," + horse_info[i]['mother_mother'] + "," + horse_info[i]['horse_id'] + "," + horse_info[i]['pre_race1'] \
                    + "," + horse_info[i]['pre_race2'] + "," + horse_info[i]['pre_race3'] + "," + horse_info[i]['pre_race4'] + "," + horse_info[i]['pre_race5'] 
    ...
    def get_race_result(race_ids):
        ...
        for i in range(0,len(race_ids)):
            ...
            records = Scraping.make_record(race_id, table_data,race_info,horse_info,jockey_info)
            #print(records)
