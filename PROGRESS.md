...
| Phase 8 | README.md 最終版作成・ディレクトリ構成見直し | ✅ 完了 |
...
### Phase 8 — README 作成・ディレクトリ構成見直し

**日付：** 2026-04-01
**ブランチ：** `feature/phase8-readme`

**対応内容：**
- `Common/`（chromedriver.exe）削除（ChromeDriverManager で自動管理のため不要）
- `Data/` → `data/`、`netKeiba/` → `netkeiba/`（小文字統一）
- `data/netkeiba/common/` → `data/netkeiba/assets/`（column.csv・encode_list.csv 等の静的参照ファイル群として意味明確化）
- `src/config.py` のパス定義を一括更新・動作確認済み
- `README.md` 作成（日本語・自分用）

**README 構成：**
- 処理フロー（6 ステップ）
- ディレクトリ構成
- セットアップ（venv・.env）
- 使い方（E2E・ステップ別・Selenium フル）
- テスト実行コマンドと結果
- 注意事項（既知バグ・モデル互換性）

**所感：**
config.py でパスを一元管理していたおかげで、3 箇所（config.py・.gitignore・ファイルシステム）の修正だけでディレクトリ名変更が完結した。Phase 2 のコンフィグ化が効いた。
