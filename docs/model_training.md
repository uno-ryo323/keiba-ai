...
### 3. 現レース結果由来の指数（データリーク防止）

`_p1`〜`_p5` サフィックスが**ない**ベース列のみ除外。当該レースが終了しないと確定しない値のため。
前走データ（`_p1`〜`_p5` 付き）は予測前から判明している正当な特徴量のため**除外しない**。
...
> `rank_offical_p1`〜`_p5` などの前走版は正当な特徴量のため使用する。
