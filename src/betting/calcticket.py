...
from ..config import ODDS_FILES, RACECARD_DIR
...
class CalcTicket:
    ...
    def __init__(self, date, race_id):
        ...
        self._base_path = RACECARD_DIR / date / race_id
        self.forecast_data = pd.read_csv(
            self._base_path / f"{race_id}_forecast.csv", sep=",", encoding="cp932"
        )
        self.tiketFile = open(self._base_path / "ticket.csv", "w")
        pd.set_option("display.max_rows", 500)

    # 単勝の購入
    def calc_Win(self):
        odds_data = pd.read_csv(
            self._base_path / ODDS_FILES["tanfuku"], sep=",", encoding="cp932"
        )
        ...
        for i in range(0, len(df_data)):
            ...
            if (1 / sum_odds) <= 1:
                out_str = (
                    ...
                    + ",,,"
                    + str(df_data.loc[i, "Win"])
                    + ","
                    + str(df_data.loc[i, "odds_Win"])
                )
        ...
        for i in range(0, len(df_data)):
            ...
            if (1 / sum_odds) <= 1:
                out_str = (
                    ...
                    + str(df_data.loc[i, "horse_gate"])
                    + ",,,"
                    + str(df_data.loc[i, "Place"])
                    + ","
                    + str(df_data.loc[i, "odds_Show_Min"])
                )
                self.tiketFile.write(out_str + "\n")
                # print(out_str)
    ...
    def calc_QuinellaPlace(self):
        odds_data = pd.read_csv(
            self._base_path / ODDS_FILES["wide"], sep=",", encoding="cp932"
        )
        odds_data = odds_data.select_dtypes(include=np.number)

        forecast_data = self.forecast_data[["horse_gate", "Place"]]
        df_data = pd.merge(
            odds_data, forecast_data, left_on="horse_gate1", right_on="horse_gate"
        )
        df_data = pd.merge(
            df_data, forecast_data, left_on="horse_gate2", right_on="horse_gate"
        )
        ...
        for i in range(0, len(df_data)):
            ...
            if (1 / sum_odds) <= 1:
                out_str = (
                    ...
                    + ","
                    + str(df_data.loc[i, "horse_gate2"])
                    + ",,"
                    + str(df_data.loc[i, "prob"])
                    + ","
                    + str(df_data.loc[i, "QuinellaPlace"])
                )
                # print(out_str)
                self.tiketFile.write(out_str + "\n")
    ...
    def calc_BracketPlace(self):
        odds_data = pd.read_csv(
            self._base_path / ODDS_FILES["wakuren"], sep=",", encoding="cp932"
        )
        if len(odds_data) <= 1:
            return
        ...
        df_data = pd.merge(
            odds_data, my_df1, left_on="lane_gate1", right_on="lane_gate"
        )
        ...
        for waku in range(1, 9):
            ...
            index = df_data.index[
                (df_data["lane_gate1"] == waku) & (df_data["lane_gate2"] == waku)
            ]
        ...
        for i in range(0, len(df_data)):
            ...
            if (1 / sum_odds) <= 1:
                out_str = (
                    ...
                    + ","
                    + str(df_data.loc[i, "lane_gate2"])
                    + ",,"
                    + str(df_data.loc[i, "prob"])
                    + ","
                    + str(df_data.loc[i, "BracketQuinella"])
                )
                self.tiketFile.write(out_str + "\n")
                # print(out_str)
    ...
    def calc_Quinella(self):
        odds_data = pd.read_csv(
            self._base_path / ODDS_FILES["umaren"], sep=",", encoding="cp932"
        )
        odds_data = odds_data.select_dtypes(include=np.number)
        ...
        for i in range(0, len(df_data)):
            ...
            if (1 / sum_odds) <= 1:
                out_str = (
                    ...
                    + str(df_data.loc[i, "prob"])
                    + ","
                    + str(df_data.loc[i, "Quinella_odds"])
                )
                # print(out_str)
                self.tiketFile.write(out_str + "\n")
    ...
    def calc_Exacta(self):
        odds_data = pd.read_csv(
            self._base_path / ODDS_FILES["umatan"], sep=",", encoding="cp932"
        )
        odds_data = odds_data.select_dtypes(include=np.number)
    ...
    def calc_Trio(self):
        odds_data = pd.read_csv(
            self._base_path / ODDS_FILES["fuku3"],
            sep=",",
            encoding="cp932",
            dtype={
                "horse_gate1": int,
        ...
        df_data = pd.merge(
            df_data, forecast_data, left_on="horse_gate3", right_on="horse_gate"
        )
        ...
        for i in range(0, len(df_data)):
            ...
            out_str = (
                "Trio,"
                + str(df_data.loc[i, "horse_gate_x"])
                + ","
                + str(df_data.loc[i, "horse_gate_y"])
                + ","
                + str(df_data.loc[i, "horse_gate"])
                + ","
                + str(df_data.loc[i, "prob"])
                + ","
                + str(df_data.loc[i, "Trio"])
            )
            # print(out_str)
            self.tiketFile.write(out_str + "\n")
    ...
    def calc_Trio2(self):
        forecast_data = pd.read_csv(
            self._base_path / f"{self.race_id}_forecast.csv", sep=",", encoding="cp932"
        )
        ...
        for data in lists:
            out_str = (
                "Trio," + str(data[0]) + "," + str(data[1]) + "," + str(data[2]) + ",,"
            )
            # print(out_str)
            self.tiketFile.write(out_str + "\n")
    ...
    def set_bettingmoney(self):
        ...
        # ticket_data[7] = 100
        ticket_data = ticket_data.to_csv(
            self._base_path / "ticket.csv", sep=",", encoding="cp932"
        )
