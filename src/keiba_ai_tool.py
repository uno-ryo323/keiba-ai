...
class keibaAITestTool(Toplevel):
    ...
    def __init__(self, master):
        ...
        self.button2 = ttk.Button(
            self.frame2,
            text="カレンダー表示",
            command=keibaAITestTool.call_popup_calender,
        )
        self.button3 = ttk.Button(
            self.frame2, text="レース一覧取得", command=self.call_get_race_list
        )

        self.scrolledText = scrolledtext.ScrolledText(
            self.frame3, height=15, state=DISABLED
        )
